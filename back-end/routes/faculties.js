...
faculties.get('/find', urlencodedParser, async (req, res) => {
        try {
            let queryResult = await DB.findFacultiesAll();
            if (queryResult.length > 0) {
                console.log("faculties found")
                res.json(queryResult)
            }
...
faculties.get('/listuniall', urlencodedParser, async (req, res) => {
    try {
        let queryResult = await DB.listUniversitiesENUM();
        if (queryResult.length > 0) {
            console.log(queryResult)
            var data = queryResult[0].COLUMN_TYPE;
            console.log(data)
            data = data.slice(6, data.length - 2);
            var unis = data.split(`','`)
            const jsonObj = []
            for (let i = 0; i < unis.length; i++) {
                jsonObj[i]={name: unis[i], key: i}
            }
            console.log("OK");
            res.json(jsonObj);
            res.status(200)
